...
public class UpdateUserPassword extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response)
		...
		String new_passsword = request.getParameter("new_passsword");
		System.out.println("用户名:	" + user_name);
		PrintWriter out = response.getWriter();
		String update_sql = "UPDATE rxpb_user_info SET password='" + new_passsword + "',update_user='" + update_user
		...
		try {
			...
			System.out.println("update user password success");
			out.println("update user password success");
			// 关闭连接
			conn.close();
			statement.close();
		} catch (SQLException se) {
			System.out.println("update user password failed");
			out.println("update user password failed");
